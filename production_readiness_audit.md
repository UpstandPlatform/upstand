...
- **Latest worker-read hardening:** Production deployment-worker callers are denied global Docker metadata and inventory (`/info`, `/images/json`, `/nodes`, and `/system/df`). Worker Swarm status now uses the bounded typed `swarm` info operation; resource-scoped service, task, network, volume, and container reads remain available only through exact filters or live ownership checks; typed inventory remains the preferred control-plane path.

...
- **Latest raw file-backed-resource hardening:** Production deployment-worker raw and typed resource-service create/update flows now inspect every Swarm `SecretID`/`SecretName` and `ConfigID`/`ConfigName` reference through the daemon before service mutation. References must resolve to the requested resource's exact ownership label or resource-scoped Compose name; named service volumes must also use the exact deterministic database or resource-Compose ownership prefix. Missing, mismatched, foreign, unbounded, and malformed references fail closed. Compose config/secret definitions now reject external resources and unsafe file/name paths, and owned definitions receive deterministic resource-scoped Docker names. Focused Go and use-case regressions cover raw create, raw update-time revalidation, typed upsert, typed cross-resource volume mounts, Compose scoping, and external/host-backed rejection. The broader raw Compose/service orchestration transport remains an open migration slice.

...
- **Latest worker-read hardening:** Production deployment-worker callers are denied global legacy image, Swarm-node, and Docker disk-usage inventory (`/images/json`, `/nodes`, and `/system/df`). Resource-scoped service, task, network, volume, and container reads remain available only through exact filters or live ownership checks; typed inventory remains the preferred control-plane path.

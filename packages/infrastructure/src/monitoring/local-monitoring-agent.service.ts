...
export async function initializeMonitoring(
  ...
  if (!monitoringInitializationPromise) {
    monitoringInitializationPromise = initializeMonitoringOnce(
      resolveSettings,
    ).catch((error: unknown) => {
...
async function initializeMonitoringOnce(
  ...
  const dockerControlNetwork = process.env.DOCKER_CONTROL_NETWORK?.trim();
  if (networkMode && dockerControlNetwork) {
    networkMode = dockerControlNetwork;
  }

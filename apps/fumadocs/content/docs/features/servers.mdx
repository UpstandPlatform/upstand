...
This is common with rootless Docker, unprivileged LXC/Incus containers, or
Docker data stored on an unsupported filesystem. Enable nesting, `keyctl`, and
the host overlayfs capability where applicable, or use a rootful Docker Engine
on ext4 or XFS with `ftype=1`; then retry setup. Do not delete Docker's data

...
graph TD
    subgraph IngressNetwork["Shared Application Ingress Network (upstand-network)"]
        Edge["OpenResty Managed Edge (:80/:443)"]
        Caddy["Caddy Application Proxy"]
        Edge --> Caddy
    end
    ...
    Caddy -->|"Application Routing"| AppA
    Caddy -->|"Application Routing"| AppB
```

### Network Security Dual-Overlay Model
1. **Shared Ingress Overlay (`upstand-network`)**: Connects the Caddy application proxy container to application services. The managed OpenResty edge uses the host listener and forwards to Caddy; it does not need to join every resource network.
...
### Advanced Runtime & Health Check Configuration

From the resource **Advanced** tab, operators can configure granular container execution settings:
- **Container Health Checks**: Define custom container probes:
  - **Test Command**: Specify a test command (e.g. `CMD-SHELL curl -f http://localhost:8080/health || exit 1`).
  - **Interval & Timeout**: Set evaluation frequency (e.g. 30s) and timeout limits (e.g. 5s).
  - **Retries & Start Period**: Configure maximum retry attempts before marking unhealthy and initial grace period for cold-starting applications.
- **Port Mappings**: Map container ports to host network ports with TCP/UDP protocol selection and optional host IP bindings (e.g. `127.0.0.1:8080:80/tcp`).
- **Volume Mounts**: Attach persistent host paths or named Docker volumes to specified container directories (e.g. `/var/lib/app/data`).

...
graph TD
    subgraph IngressNetwork["Shared Application Ingress Network (upstand-network)"]
        Caddy["Caddy Application Proxy (:80/:443)"]
    end
...
1. **Shared Ingress Overlay (`upstand-network`)**: Connects the Caddy application proxy container to application services. Caddy owns the public listeners and routes traffic to the selected resource without joining every resource network.

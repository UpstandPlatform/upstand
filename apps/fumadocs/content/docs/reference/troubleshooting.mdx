...
flowchart TD
    ...
    Category -->|Container Startup Failure| ServiceTree[2. Service Startup Failures]
    ServiceTree --> LogCheck[Run: docker service ps upstand_server]

    Category -->|SSL / HTTPS Certificate Error| CertTree[3. Caddy Certificate Failures]
    CertTree --> PortCheck[Check Ports 80 & 443 with ss -ltnp]

    Category -->|Build Job Pending / Stuck| QueueTree[4. Stuck Build Queues]
    QueueTree --> QueueCheck[Inspect Redis Queue & docker node ls]

    Category -->|Lost Admin 2FA Token| TwoFATree[6. Reset 2FA CLI]
...
## 6. Database Schema Generation on Windows
...
## 7. Resetting Two-Factor Authentication (2FA)

...
| `UPSTAND_ALLOW_INSECURE_BOOTSTRAP` | No | Allows an installation with no configured HTTPS origins to complete its initial direct-IP bootstrap. It does not disable the permanent direct IP:port break-glass path. |
...
| `UPSTAND_DIRECT_ORIGINS` | Legacy compatibility variable; direct same-IP HTTP origins are now always trusted for control-plane recovery. Keep firewall/VPN policy restrictive. |
...
| `UPSTAND_MONITORING_SOURCE_DIR` | Desktop-local runtime path to the bundled monitoring source used to build the agent on a configured remote server. Self-hosted and cloud production runtimes must use `UPSTAND_MONITORING_IMAGE` instead. |
...
| `UPSTAND_DR_READINESS_GATE` | Installer-managed | Enables installation-specific disaster-recovery attestation checks in production acceptance. Production Compose defaults this to `true`; disposable release environments must explicitly set it to `false`. |
...
| `UPSTAND_DOCKER_BROKER_DEV_INSECURE` | Development-only | Explicitly opts the standalone broker into its legacy insecure mode. Never set this in production. When unset, strict identity and caller-scoped authorization are enabled by default. |
...
## Installer and nested-runtime variables

| Variable | Purpose |
| --- | --- |
| `UPSTAND_BUILD_FROM_SOURCE` | Explicitly selects a source build instead of the immutable release manifest. |
| `UPSTAND_REF` | Git branch or tag used for a source build. |
| `UPSTAND_ALLOW_DOCKER_INSTALL` | Explicitly allows the installer to invoke the upstream Docker installation script when Docker is absent. |
| `UPSTAND_ALLOW_SINGLE_REPLICA` | Required acknowledgement for the bundled single-replica control plane and data services. |
| `UPSTAND_ALLOW_UNDERSIZED_HOST` | Recovery/test-only override for hosts below the recommended 2 vCPU, 4 GiB RAM, or 30 GiB storage. |
| `SWARM_ADVERTISE_ADDR` | Explicit IPv4/IPv6 address used by `docker swarm init`; otherwise the installer discovers a routable address. |
| `UPSTAND_SECRET_PROVIDER_ALLOWED_HOSTS` | Optional comma-separated provider host allowlist for private Vault, Infisical, and other provider endpoints. URL and DNS safety checks still apply. |
| `UPSTAND_DOCKER_CONTROL_NETWORK` | Internal encrypted control-network name used by the Docker broker. |

Interactive mode can collect these settings from `/dev/tty`, including when
the installer is streamed through `curl | sudo bash`. Non-interactive CI or
automation must provide the required values explicitly.
...
| `UPSTAND_AUTH_EMAIL_CHANNEL_ID` | Required in cloud mode | Organization notification-channel ID used for Better Auth email verification. Cloud sign-up fails closed when it is absent or invalid. |
| `UPSTAND_METRICS_TOKEN_FILE` | Optional production secret-file path | Bearer token file for the internal metrics endpoint. If the file is missing or empty, production metrics remain disabled and the server logs a non-secret diagnostic. |
...
The installer always keeps the control-plane services available on the host IP
and their published ports. Configure the domain and HTTPS certificate in **Web
Server** for normal use, but do not expect that route to remove direct access:
the API and dashboard IP:port path is the permanent break-glass route for DNS,
Caddy, and certificate failures. Restrict the direct ports with the host
firewall, a private network, or a VPN rather than disabling them in the UI.

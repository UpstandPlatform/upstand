...
sequenceDiagram
    ...
    participant Agent as UpGal AI Agent Loop
    participant Auth as Capability Authorization
    participant DB as Upstand System DB

    User->>UI: "Deploy my app and clean unused Docker images"
    UI->>Agent: Send Message (User Session & Org Scope)
    Agent->>Auth: Evaluate User Capabilities (resource:update, docker:clean)
    Auth-->>Agent: Capabilities Authorized

    Agent->>UI: Stream Step: Requesting Approval to deploy 'app-123'
    User->>UI: Clicks "Approve Action" Button
    UI->>Agent: Send Signed Approval Response Token
    Agent->>DB: Execute Mutation (Deploy App & Prune Images)
    DB-->>Agent: Action Succeeded (Deployment Queued)
    Agent-->>UI: Stream Final Response & Action Status
```
...
The quickest setup is available in **Settings → UpGal Settings → MCP server**. The panel resolves the correct `/api/mcp` origin for the current cloud, self-hosted, or desktop-local runtime, creates a scoped read-only key, and provides copy-ready snippets for supported clients. The secret is returned once by the API and is only held in the page while the configuration is visible; revoke or rotate it from **Settings → API Keys**.

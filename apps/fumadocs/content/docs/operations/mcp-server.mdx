...
Administrators can use **Settings → UpGal Settings → MCP server** to create a read-only MCP key and select a client tab. The panel generates runtime-aware, copy-ready snippets for Claude Code, Codex, Gemini/Antigravity, Cursor, VS Code/Copilot, and generic HTTP MCP clients. It works with desktop-local, cloud, and self-hosted Upstand instances without hard-coding a deployment origin.

...
Start with the [installation guide](/docs/getting-started/installation). It
covers self-hosted and cloud modes, release channels, native and nested Linux
hosts, interactive installation, secure upgrades, firewall checks, and
Dokploy-compatible external secret references.

For production, pin an audited release tag before downloading the installer:
...
Do not use mutable image tags, plaintext database credentials, or a direct
Docker Compose deployment for production. The installer requires explicit
acknowledgements for the single-replica operating model, observability, and
disaster-recovery plan before it grants the control plane access to Docker.

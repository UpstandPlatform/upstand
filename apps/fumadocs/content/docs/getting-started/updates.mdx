...
3. **Edit variables**: Modify the following variables in `/etc/upstand/.env`:
   ```bash
   UPSTAND_SERVER_IMAGE=ghcr.io/upstandplatform/upstand-server@sha256:<new-digest>
   UPSTAND_SCHEDULES_IMAGE=ghcr.io/upstandplatform/upstand-schedules@sha256:<new-digest>
   UPSTAND_DEPLOYMENT_WORKER_IMAGE=ghcr.io/upstandplatform/upstand-deployment-worker@sha256:<new-digest>
   UPSTAND_MONITORING_IMAGE=ghcr.io/upstandplatform/upstand-monitoring@sha256:<new-digest>
   UPSTAND_WEB_IMAGE=ghcr.io/upstandplatform/upstand-web@sha256:<new-digest>
   UPSTAND_DOCS_IMAGE=ghcr.io/upstandplatform/upstand-fumadocs@sha256:<new-digest>
   UPSTAND_VERSION=v0.2.6 # Update matching release version
   ```

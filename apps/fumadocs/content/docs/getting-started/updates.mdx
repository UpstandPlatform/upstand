...
sequenceDiagram
    ...
    GH-->>API: Return Release Manifest (vX.Y.Z)
...
| --- | --- | --- |
| **Stable** | Audited Tags (`vX.Y.Z`) | Production environments. Uses a release manifest and immutable image digests. |
| **Canary** | `canary` ref | Test/staging environments. Uses an explicit source build and is not a verified release artifact. |
...
3. **Select the release**: Export the exact audited tag, then download and run that release's installer:
   ...
   ```
4. **Wait for rollout**: The installer creates a protected pre-upgrade archive under `/etc/upstand/backups/`, resolves the selected release manifest, updates the saved application image pins, and waits for migrations and service readiness. Availability during rollout depends on capacity, replica counts, and migration compatibility.

...
<Steps>
  <Step>
    ...
    Configure the HTTPS origins before running the installer. It will generate cryptographic secrets under `/etc/upstand/secrets/`, resolve the latest stable release, and pull its prebuilt GHCR images. Source builds require the explicit `UPSTAND_BUILD_FROM_SOURCE=true` opt-in.

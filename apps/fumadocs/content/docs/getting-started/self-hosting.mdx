...
After the first boot, open the dashboard at `http://<server-ip>:3001`. The API is at `http://<server-ip>:3000` and documentation is at `http://<server-ip>:4000`. On **Web Server**, configure the server domain and HTTPS certificate. Once that domain is working over HTTPS, clear **Direct IP:port access** and save; Upstand then removes the published control-plane ports while keeping the Caddy domain route active.

...
<Steps>
  <Step>
    ...
    You can run the installer with no environment variables. It will generate cryptographic secrets under `/etc/upstand/secrets/`, detect a usable host address, and build the release images from the public repository when image digests are not supplied. Missing origins default to the host IP and the published service ports; no synthetic DNS names are assigned.

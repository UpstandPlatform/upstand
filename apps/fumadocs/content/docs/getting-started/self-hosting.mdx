...
<Steps>
  <Step>
    ...
    Configure the HTTPS origins before running the installer. It will generate cryptographic secrets under `/etc/upstand/secrets/`, resolve the latest stable release, verify its immutable GitHub release manifest and deployment-artifact hashes, and pull the exact published GHCR image digests. Releases predating deployment-artifact hashes are rejected unless you explicitly set `UPSTAND_ALLOW_LEGACY_RELEASE_ARTIFACTS=true` after reviewing the fetched files. Source builds require the explicit `UPSTAND_BUILD_FROM_SOURCE=true` opt-in.
...
Production installs expose only Caddy on `:80/:443`; the API, dashboard, scheduler, and database services remain on the private Swarm network. Configure `BETTER_AUTH_URL`, `CORS_ORIGIN`, and `NEXT_PUBLIC_SERVER_URL` as HTTPS origins before installation. An insecure direct-IP bootstrap requires the explicit `UPSTAND_ALLOW_INSECURE_BOOTSTRAP=true` opt-in and is intended only for isolated development hosts.
...
# The production stack always mounts these optional URL secrets. Leave them
# empty for bundled PostgreSQL/Redis; the installer writes real values when
# external HA data services are configured.
: | sudo tee /etc/upstand/secrets/database_url >/dev/null
: | sudo tee /etc/upstand/secrets/redis_url >/dev/null

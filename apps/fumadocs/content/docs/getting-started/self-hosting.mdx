...
graph TD
    ...
    subgraph Host["Production Host / Virtual Machine"]
        subgraph IngressLayer["Public Ingress (Port 80 / 443)"]
            Edge["OpenResty Managed Edge"]
            Caddy["Caddy Application Proxy (Compatibility Backend)"]
        end
...
4. **Firewall Rules**:
   - Open public TCP **80** and **443** for the managed OpenResty edge. Caddy remains behind the edge on internal backend ports and keeps its existing domain-routing behavior.
   - Until a domain is configured, the dashboard, API, and documentation are also reachable on TCP **3001**, **3000**, and **4000** respectively.
...
<Steps>
  <Step>
    ...
    You can run the installer with no environment variables. It will generate cryptographic secrets under `/etc/upstand/secrets/`, detect a usable host address, resolve the latest stable release, and pull its prebuilt GHCR images. Missing origins default to the host IP and the published service ports; no synthetic DNS names are assigned. Source builds require the explicit `UPSTAND_BUILD_FROM_SOURCE=true` opt-in.
...
After the first boot, open the dashboard at `http://<server-ip>:3001`. The API is at `http://<server-ip>:3000` and documentation is at `http://<server-ip>:4000`. On **Web Server**, configure the server domain and HTTPS certificate. The managed edge owns public `:80/:443` and forwards legacy Caddy mappings to the backend; explicit edge routes can add policy before Caddy. Once the domain is working over HTTPS, clear **Direct IP:port access** and save; Upstand then removes the published control-plane ports while keeping the Caddy domain route active.
...
```bash
curl -fsSL https://raw.githubusercontent.com/UpstandPlatform/upstand/master/install.sh | sudo bash -s -- --interactive
```
...
```bash
sudo curl -fsSL https://raw.githubusercontent.com/UpstandPlatform/upstand/master/docker-compose.prod.yml -o /etc/upstand/docker-compose.yml

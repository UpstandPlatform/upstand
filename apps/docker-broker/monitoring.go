...
func (engine *dockerEngineClient) ensureTypedMonitoringNetwork(ctx context.Context, name string) error {
	...
	if network.Name != name || network.Driver != `overlay` || network.Scope != `swarm` || !network.Attachable || !network.Internal || !networkEncryptionAllowed(network.Options, allowUnencryptedNetworkForAcceptance()) {
